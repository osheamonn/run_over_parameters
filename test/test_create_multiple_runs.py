...
def test_create_runs():
    with create_test_workflow():
        ...
        parameters = pd.read_csv(relative_to_absolute_path(
            "TestOutput/parameters.csv"), sep='\t')

        assert len(run_dirs) == 9
        assert len(parameters) == 9

        assert filecmp.cmp(relative_to_absolute_path(
            'test_files/expected_parameters.csv'), \
                           os.path.join(
                               relative_to_absolute_path('TestOutput/parameters.csv')),
                           shallow=False)
        ...
        for d in run_dirs:
            ...
            assert filecmp.cmp(relative_to_absolute_path(relative_to_absolute_path(
                'test_files/sample.py')),
                               relative_to_absolute_path(os.path.join(d,
                                                                      'renamed_sample.py')))
        # Test submission
        submit()
